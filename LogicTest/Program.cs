...
namespace LogicTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] words = TextoB.Split(' ');

            char[] letrasFoo = { 's', 'l', 'f', 'w', 'k' };

            string alfabeto = "kbwrqdnfxjmlvhtcgzps";

            counter c = new counter(words, letrasFoo, alfabeto);


            int p = c.countPrepositions();
            int v = c.countVerbs().Item1;
            int fpv = c.countVerbs().Item2;
            Console.WriteLine("Preposições: "+ p +"\nVerbos: "+ v +"\nVerbos em primeira pessoa: "+ fpv);


            string[] organizedText = c.Organizer();
            Console.WriteLine("\nLista de palavras em ordem:\n");
            foreach(var word in organizedText)
            {
                Console.Write(word+" ");
            }


            int bonitos = c.Bonito();
            Console.WriteLine("\n\nQuantidade de números bonitos: "+ bonitos +"\n");
    ...
    class counter
    {
        ...
        public counter(string[] s, char[] c, string a)
        {
            words = s;
            letters = c;
            alphabet = a;
        ...
        public (int,int) countVerbs()
        {
            ...
            return (verbs,fPVerbs);
        }

        public string[] removeRepetition()
        {
            ...
            foreach (string word in words)
            {
                ...
                {
                    if (Convert.ToString(alist[i]) != word)
                    {
                        if (i == alist.Count - 1)
                        {
                            alist.Add(word);
            ...
            {
                list[i] = Convert.ToString(alist[i]);
            }
            return list;
        }
        ...
        public string[] Organizer()
        {
            ...
            while (change == true)
            {
                ...
                for (int i = 0; i < text.Length - 1; i++)
                {
                    ...
                    {
                        loops = text[i + 1].Length;
                    }

                    for (int j = 0; j < loops; j++)
                    {

                        if (alphabet.IndexOf(text[i][j]) < alphabet.IndexOf(text[i + 1][j]))
                        {
                            j = loops;
                        }

                        else if (alphabet.IndexOf(text[i][j]) > alphabet.IndexOf(text[i + 1][j]))
                        {
                            xchange = text[i];
                            text[i] = text[i+1];
                            text[i+1] = xchange;
        ...
        public int Bonito()
        {
            ...
            for(int i=0;i < list.Length;i++)
            {
                ...
                for(int j=0;j < list[i].Length;j++)
                {
                    ...
                    soma += Convert.ToUInt64(alphabet.IndexOf(list[i][j]) * Math.Pow(20, j));
